...
impl SkillEffect {
    ...
    fn fire_hook(&mut self, hook: condition::Hook, fight: &Fight, managers: &mut Managers, entity_uid: i64) -> Vec<Event> {
        ...
        let matching: Vec<_> = self.slots.iter_mut()
            .filter(|s| s.matches_hook(hook) && s.within_limits() && s.check_conditions(owner_uid, eval))
            .flat_map(|s| {
                s.use_count += 1;
                s.round_use_count += 1;
                s.behaviours.iter().map(|b| (b.raw.clone(), b.target)).collect::<Vec<_>>()
            })
            .collect();
        matching.into_iter()
