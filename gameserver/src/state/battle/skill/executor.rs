...
mod tests {
    ...
    fn dmg_effect(target: i64) -> ActEffect {
        ActEffect {
    ...
    fn nondmg_effect(target: i64) -> ActEffect {
        ActEffect {
    ...
    #[test]
    fn damage_targets_empty() {
        assert!(damage_targets_in(&[]).is_empty());
    }

    #[test]
    fn damage_targets_single() {
        let effects = vec![dmg_effect(42)];
        assert_eq!(damage_targets_in(&effects), vec![42]);
    }

    #[test]
    fn damage_targets_dedup() {
        let effects = vec![dmg_effect(42), dmg_effect(42), dmg_effect(7)];
        assert_eq!(damage_targets_in(&effects), vec![42, 7]);
    }

    #[test]
    fn damage_targets_skip_non_damage() {
        let effects = vec![nondmg_effect(99), dmg_effect(42)];
        assert_eq!(damage_targets_in(&effects), vec![42]);
    }

    #[test]
    fn damage_targets_skip_zero_and_none() {
        let effects = vec![
            ActEffect {
                effect_type: Some(EffectType::Damage as i32),
                target_id: None,
        ...
        assert_eq!(damage_targets_in(&effects), vec![7]);
    }

    #[test]
    fn is_ex_skill_for_known_ex_returns_true() {
        ...
        let row = cfg
            ...
            .iter()
            .find(|c| c.ex_skill > 0)
            .expect("at least one character should have ex_skill > 0");
        assert!(is_ex_skill(row.ex_skill), "ex_skill {} should classify as EX", row.ex_skill);
    }

    #[test]
    fn is_ex_skill_zero_and_negative_are_false() {
        ensure_game_data_initialized();
        assert!(!is_ex_skill(0));
        assert!(!is_ex_skill(-1));
    }
}
