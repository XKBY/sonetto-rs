...
pub fn on_enter_fight(managers: &mut Managers, fight: &Fight, entity_uid: i64) -> Vec<Event> {
    ...
    let buffs = snapshot.buff_mgr.active_buff.get(&entity_uid).cloned().unwrap_or_default();
    let mut events = snapshot.cloth_mgr.on_enter_fight(fight, entity_uid);
    events.extend(BuffMgr::on_enter_fight(&buffs, fight, &mut snapshot, entity_uid));
    let rule_effects = std::mem::take(&mut snapshot.rule_mgr.effects);
    events.extend(RuleMgr::on_enter_fight(&rule_effects, fight, &mut snapshot, entity_uid));
    snapshot.rule_mgr.effects = rule_effects;
    *managers = snapshot;
    events
}
...
pub fn on_use_card(managers: &mut Managers, fight: &Fight, events: Vec<Event>, entity_uid: i64) -> Vec<Event> {
    let mut snapshot = managers.clone();
    let buffs = snapshot.buff_mgr.active_buff.get(&entity_uid).cloned().unwrap_or_default();
    let mut events = snapshot.cloth_mgr.on_use_card(fight, events);
...
pub fn on_move_card(managers: &mut Managers, fight: &Fight, events: Vec<Event>, entity_uid: i64) -> Vec<Event> {
    let mut snapshot = managers.clone();
    let buffs = snapshot.buff_mgr.active_buff.get(&entity_uid).cloned().unwrap_or_default();
    let mut events = snapshot.cloth_mgr.on_move_card(fight, events);
...
pub fn on_compose_card(managers: &mut Managers, fight: &Fight, events: Vec<Event>, entity_uid: i64) -> Vec<Event> {
    let mut snapshot = managers.clone();
    let buffs = snapshot.buff_mgr.active_buff.get(&entity_uid).cloned().unwrap_or_default();
    let mut events = snapshot.cloth_mgr.on_compose_card(fight, events);
...
pub fn on_dead(managers: &mut Managers, fight: &Fight, entity_uid: i64) -> Vec<Event> {
    ...
    events.extend(snapshot.cloth_mgr.on_dead(fight, entity_uid));
    events.extend(BuffMgr::on_dead(&buffs, fight, &mut snapshot, entity_uid));
    let rule_effects = std::mem::take(&mut snapshot.rule_mgr.effects);
    events.extend(RuleMgr::on_dead(&rule_effects, fight, &mut snapshot, entity_uid));
    snapshot.rule_mgr.effects = rule_effects;
    *managers = snapshot;
    events
}
...
pub fn on_buff_add(managers: &mut Managers, fight: &Fight, target_uid: i64) -> Vec<Event> {
    let mut snapshot = managers.clone();
    let buffs = snapshot.buff_mgr.active_buff.get(&target_uid).cloned().unwrap_or_default();
